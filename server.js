...
async function consturctServer(moduleDefs) {
  ...
  const FAKE_IP = '116.25.146.177'; // 伪装成深圳电信的固定 IP

  app.use((req, res, next) => {
    try {
      // 强制修改请求头，注入国内 IP，绕过海外风控
      req.headers['x-real-ip'] = FAKE_IP;
      req.headers['x-forwarded-for'] = FAKE_IP;

      if (req.path && req.path.indexOf('/song/url') !== -1) {
        // 直接将身份注入到最底层的 headers 中
